...
function CotizacionMaritima() {
    ...
    function savei(e, word) {
        ...
        let obj = pdfData && Object.entries(pdfData).reduce((ac, i, index) => {
            return i[0].includes(word === 'incluye' ? 'CM-INCLUYE' : 'CM-EXCLUYE') ? { ...ac, ...{ [i[0]]: i[1] } } : ac
        }, {})

        word === 'incluye'
            ? writeUserData('/cm-incluye', obj, setUserSuccess)
            : writeUserData('/cm-excluye', obj, setUserSuccess)
    }

    function complete(e, word) {
        e.preventDefault()
        word === 'incluye'
            ? setUserPdfData({ ...pdfData, ...userDB['cm-incluye'] })
            : setUserPdfData({ ...pdfData, ...userDB['cm-excluye'] })
    }
    function generateNO() {
