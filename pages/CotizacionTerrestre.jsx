...
function CotizacionTerrestre() {
    ...
    function savei(e, word) {
        ...
        let obj = pdfData && Object.entries(pdfData).reduce((ac, i, index) => {
            return i[0].includes(word === 'incluye' ? 'CT-INCLUYE' : 'CT-EXCLUYE') ? { ...ac, ...{ [i[0]]: i[1] } } : ac
        }, {})

        word === 'incluye'
            ? writeUserData('/ct-incluye', obj, setUserSuccess)
            : writeUserData('/ct-excluye', obj, setUserSuccess)
    }

    function complete(e, word) {
        e.preventDefault()
        word === 'incluye'
            ? setUserPdfData({ ...pdfData, ...userDB['ct-incluye'] })
            : setUserPdfData({ ...pdfData, ...userDB['ct-excluye'] })
    }
    function generateNO() {
        let cotizacionNo = userDB.CotizacionTerrestre
